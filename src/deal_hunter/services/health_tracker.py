"""Health tracking service for Deal Hunter.

Wraps all health monitoring logic: load/save health.json,
compute overall status, track per-source failures, and provide
CLI status/watchdog output.
"""
...
class HealthTracker:
    """Tracks run health, source failures, and provides watchdog checks."""
    ...
    def load(self) -> dict | None:
        ...
        if not self.health_file.exists():
            return None
        try:
            with self.health_file.open(encoding="utf-8") as f:
    ...
    @staticmethod
    def _compute_status(profile_results: dict) -> str:
    ...
    def update_sources(
        ...
        for source_name, success in source_results.items():
            ...
            if success:
                ...
                status = (
                    "degraded" if consecutive < self.CONSECUTIVE_FAILURE_ALERT_THRESHOLD else "down"
                )
                sources_health[source_name] = {
                    "status": status,
    ...
    def build_data(
        ...
        return {
            ...
            "status": self._compute_status(profile_results),
    ...
    def get_failing_sources(sources_health: dict) -> list[str]:
        ...
        return [
            ...
            >= HealthTracker.CONSECUTIVE_FAILURE_ALERT_THRESHOLD
        ]
    ...
    def print_status(self) -> int:
        ...
        if age > self.STALE_THRESHOLD:
            ...
            print(
                f"\u274c STALE \u2014 last run was {age_str} ago "
                f"({last_run.strftime('%Y-%m-%d %H:%M')})"
            )
            return 3

        status = health.get("status", "error")
        status_icon = {"ok": "\u2705", "partial": "\u26a0\ufe0f", "error": "\u274c"}.get(
            status, "\u2753"
        )
        age_str = self._format_timedelta(age)
    ...
    def check_watchdog(self) -> tuple[bool, str]:
