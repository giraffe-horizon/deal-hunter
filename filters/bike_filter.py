...
class BikeFilter(BaseFilter):
    ...
    def score_deal(self, deal) -> ScoreResult:
        ...
        if size_result == "good":
            result.score += 10
            result.plus.append("+10 dobry rozmiar")
        elif size_result == "unknown":
            result.score -= 30
            result.minus.append("-30 brak info o rozmiarze")

        # Color exclusion
        for color in self.excluded_colors:
            if color.lower() in text:
                result.score -= 100
                result.minus.append(f"-100 kolor {color}")
        ...
        if tire_match:
            ...
            if 38 <= tire_width <= 50:
                result.score += 20
                result.plus.append(f"+20 opona {tire_width}mm (ideał)")
            elif 32 <= tire_width <= 37:
                result.score += 10
                result.plus.append(f"+10 opona {tire_width}mm (OK)")
            elif 23 <= tire_width <= 27:
                result.score -= 10
                result.minus.append(f"-10 wąska opona {tire_width}mm")
        ...
        if race_count > 0:
            ...
            result.minus.append(f"{penalty} wyścigowe słowa kluczowe (x{race_count})")
