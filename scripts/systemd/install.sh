...
for unit in deal-hunter.service deal-hunter.timer \
            ...
            deal-hunter-digest.service deal-hunter-digest.timer \
            deal-hunter-bot.service \
...
systemctl --user enable --now deal-hunter-digest.timer

echo "Enabling and starting feedback bot ..."
systemctl --user enable --now deal-hunter-bot.service

echo ""
echo "Done! Check status with:"
echo "  systemctl --user status deal-hunter.timer"
echo "  systemctl --user status deal-hunter-watchdog.timer"
echo "  systemctl --user status deal-hunter-digest.timer"
echo "  systemctl --user status deal-hunter-bot.service"
